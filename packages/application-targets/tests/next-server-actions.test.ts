...
describe("next-server-actions adapter tests", () => {
  beforeAll(() => {
    registerAllBuiltInAdapters();
  });

  it("compiles with next-server-actions transport successfully", async () => {
    ...
    const config = {
      ...
      targets: [
        {
          id: "next-actions-target",
          adapter: "next-server-actions",
          mode: "scaffold" as const,
          outputRoot: "generated/next-actions-target",
    ...
    expect(res.fileSet?.files.some(f => f.path === "src/generated/server-actions.ts")).toBe(true);
  });
});
